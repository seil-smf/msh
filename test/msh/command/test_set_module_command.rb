...
class SetModuleCommandTest < Test::Unit::TestCase
  def setup
    $conf = {
  ...
  def test_no_option
    ...
    c = Msh::Command::SetModuleCommand.new
    c.expects(:execute).returns(response)

    c.doit(["set", "module", "tsw00000000", "0", "1.0.0-0"])

expected_str = <<EOS
module: Module version was updated to 1.0.0-0.
EOS
