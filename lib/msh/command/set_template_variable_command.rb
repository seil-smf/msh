# -*- coding: utf-8 -*-

require 'msh/command/abstract_command'

module Msh
  module Command
    class SetTemplateVariableCommand < AbstractCommand
      def doit(command_array)
        @verbose = command_array.delete('-v')
        command_args = CommandArgs.new
        command_args.parse_args(command_array)
        return if command_args.has_error?
        command_args.parse_optional_args(command_array)
        set_request_param(command_args)

        api = Msh::Api::PUTUserUserCodeTemplateIdVariableName.new({
                                                                    :id => command_args[:template_id],
                                                                    :name => command_args[:template_variable_name],
                                                  })
        api.request = @request[:request]

        response = execute(api)
        return unless check_http_success(response)

        if @verbose
          puts_response(response)
        else
          template_variable = json_load(response.body)
          $output.puts "---"
          $output.puts "TemplateVariable Name: #{template_variable["name"]}"
          $output.puts "TemplateVariable DefaultValue: #{template_variable["defaultValue"]}"
          $output.puts "Values:"
          template_variable["values"].each_with_index do |value, i|
            $output.puts "  - Value_#{i}:"
            $output.puts "      SA Code: #{value["code"]}"
            $output.puts "      Value: #{value["value"]}"
          end
        end
      end

      private

      def set_request_param(request_param)
        @request[:request] = {}
        @request[:request][:defaultValue] = request_param[:default_value] if request_param[:default_value]
        @request[:request][:values] = request_param[:values] if request_param[:values]
      end

      class CommandArgs < Hash
        include Msh::AbstractCommandArgs

        def parse_args(args)
          self.merge!({
                        :command      => args.shift,
                        :subcommand   => args.shift,
                        :template_id =>  args.shift,
                        :template_variable_name => args.shift,
                      })
        end

        def parse_optional_args(args)
          if idx = args.index('defaultvalue')
            args.shift
            self[:default_value] = args.shift if args[idx]
          end

          if idx = args.index('values')
            args.delete_at(idx)
            values = []
            (args.size / 2).times do |i|
              values << {
                :code => args.delete_at(idx),
                :value => args.delete_at(idx)
              }
            end

            self[:values] = values unless values.empty?

          end

        end
      end

    end
  end
end
