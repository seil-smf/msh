...
module Msh
  module Command
    class PullConfigCommand < AbstractCommand
      def doit(command_array)
        ...
        clear_request
        set_api("/user/#{$conf[:user_code]}/request/pull-config")
        set_method(:POST)
        ...
        pull_config_id = json_load(response.body)['id']

        clear_request
        set_api("/user/#{$conf[:user_code]}/request/pull-config/#{pull_config_id}")
        set_method(:GET)
      ...
      def set_request_param(request_param)
        @request[:request] = {
          ...
          :pullType => request_param[:pulltype]
        }
      end
